...
namespace CociMundo
{
    class cGrafo
    {
        ...
        public void AgregarArista(string vertice_1, string vertice_2, int peso)
        {  //agregamos un camino entre dos nodos
        ...
        Dictionary<string, int> Dijkstra(string nodo_partida)
        {
            ...
            for (int n = 0; n < this.Grafo.Count(); n++)
            {
                ...
                foreach (var (nodo, peso) in this.Grafo[nodo_actual])
                {  //me guardo el mejor camino 
        ...
        string Min_distancia(HashSet<string> Lo_que_ya_visite, Dictionary<string, int> dist_hasta)
        {
            ...
            {
                if (!Lo_que_ya_visite.Contains(item.Key) && dist_hasta[item.Key] < min)
                {  //recorro la lista de distancias desde el nodo de partida y me guardo la minima (y el nodo de destino que le corresponde
